...
//        multipleSeriesRenderer.setChartTitleTextSize(30);//设置整个图表上方标题文字的大小
        ...
        multipleSeriesRenderer.setAxesColor(Color.parseColor("#AEB0B9"));//设置轴的颜色
        multipleSeriesRenderer.setAxisTitleTextSize(17);//设置轴文字标注的大小
        multipleSeriesRenderer.setLabelsColor(Color.parseColor("#AEB0B9"));//设置轴文字标注的颜色
//        multipleSeriesRenderer.setRange(new double[]{0, 4, 0, maxY });//xy轴的刻度范围
        ...
        multipleSeriesRenderer.setYLabels(6);//Y轴刻度等份
        multipleSeriesRenderer.setXLabels(0);//X轴刻度等份

        multipleSeriesRenderer.setLabelsTextSize(16);//设置横纵坐标刻度值的大小
        multipleSeriesRenderer.setXLabelsColor(Color.parseColor("#AEB0B9"));//横坐标上面刻度值的颜色
        multipleSeriesRenderer.setYLabelsColor(0,Color.parseColor("#AEB0B9"));//纵坐标上面刻度值的颜色
...
//        multipleSeriesRenderer.setLegendTextSize(15);//设置图例文字大小
        ...
        r.setColor(Color.parseColor("#AEB0B9"));
    ...
    public void updateChart(double addY){
        ...
        int data = getScaleNum((count-1)*xDate).intValue();
        if (length < 20) {
            ...
            for(int i = 0; i< Y.size(); i++){
                int d = getScaleNum((count-length+i)*xDate).intValue();
                multipleSeriesRenderer.addXTextLabel(d,i +"");
                //每新增一个点，就将刻度值往左平移一个刻度距离，使横轴刻度随着点X坐标值动态改变
                if (Y.size() > i+1) {
